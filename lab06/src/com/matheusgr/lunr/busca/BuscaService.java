...
public class BuscaService {
	...
	public BuscaService(DocumentoService ds) {
		if (ds == null) {
			throw new NullPointerException("O documento service não pode ser nulo.");
		}
	...
	/**
	 * Realiza uma operação de busca avançada. Toda busca deve retornar todos os documentos 
	 * que possuem todos os metadados utilizados na busca.
	 * 
	...
	public DocumentoDTO[] busca(BuscaAvancada buscaAvancada) {
		...
		DocumentoDTO[] documentos = respostaDocumento.entrySet().stream().map(Entry::getKey)
		.map(DocumentoDTO::new).collect(Collectors.toList()).toArray(new DocumentoDTO[] {});
